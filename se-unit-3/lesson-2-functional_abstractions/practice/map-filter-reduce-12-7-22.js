...
/* 
Array.prototype.forEach executes a callback function
on each value of an array. The callback will be provided
...
// 2: for loop calling a function on each element
// for (let i = 0; i < fellows.length; i++) {
...
// 3: forEach, passing in the function
// fellows.forEach(addMadFellow);

// 4: forEach, inline arrow function
fellows.forEach(currentFellow => {
...
//   if (fellows[i].toLowerCase().includes("c")) {
...
function headAss(string) {
    ...
    // Make a variable to act as a switch between uppercase and lowercase
    // For each character
    // - if the switch is uppercase, make the character uppercase
    // - if the switch is lowercase, make the character lowercase
    // - flip the switch
    // join the array back into a string and return the string
}

const headAssTestValues = ['abcdef', 'Abc+def', 'Omar Apollo is better than Frank Ocean']
headAssTestValues.forEach(value => console.log(headAss(value)))
...
/* 
Array.prototype.map executes a callback function
on each value of an array. The callback will be provided
with three arguments: element, index, array. 

The result of each callback invocation will be stored 
in a new array that is returned.

What is the difference between forEach and map?
* map returns a new array, forEach returns undefined
* map uses the returned value from the callback invocation, forEach doesn't
*/

const numsToBeSquared = [1,2,3,4,5];
const squared = []; // [1, 4, 9, 16, 25]

// for (let i = 0; i < numsToBeSquared.length; i++) {
//   const num = numsToBeSquared[i];
//   squared.push(num * num);
// }

numsToBeSquared.forEach(num => {
  squared.push(num * num)
});

const squaredByMap = numsToBeSquared.map(num => {
  return num * num;
});
console.log('squared:',squared)
console.log('squaredByMap:',squaredByMap)



const words = ['dog', 'cat', 'worm', 'kyle'];
const pluralized = []; // ['dogs', 'cats', 'worms', 'kyles']
for (let i = 0; i < words.length; i++) {
  const word = words[i];
  pluralized.push(word + 's')
}

const pluralizedByMap = words.map(word => {
  return word + 's'
})
console.log(pluralizedByMap);
...
firstNames.push(users[0].firstName)
firstNames.push(users[1].firstName)
firstNames.push(users[2].firstName)

for (let i = 0; i < users.length; i++) {
  let user = users[i];
  firstNames.push(users.firstName)
}

users.forEach(user => firstNames.push(user.firstName))
const firstNamesByMap = users.map(user => user.firstName);

console.log(firstNamesByMap)
...
/* 
Array.prototype.filter executes a callback function
on each value of an array. The callback will be provided
with three arguments: element, index, array. 
...
const nums = [1, 5, 12, 18, 94, 3, 16];
const numsLessThanTen = [];
for (let i = 0; i < nums.length; i++) {
  const num = nums[i];
  if (num < 10) {
    numsLessThanTen.push(num);
  }
}
